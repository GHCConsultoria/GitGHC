...
const IconeRelogio = (p: SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 20 20" fill="none" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" {...p}>
...
export function PainelResumo({
  ...
  const cartoes: CartaoResumo[] = [
    { rotulo: "Vencendo hoje", valor: vencendoHoje, Icone: IconeRelogio, acento: "border-l-urgent-line", corIcone: "bg-urgent-bg text-urgent" },
    { rotulo: "Aguardando confirmação", valor: aguardandoConfirmacao, Icone: IconeAguardando, acento: "border-l-attention-line", corIcone: "bg-attention-bg text-attention" },
    { rotulo: "Confirmados", valor: confirmados, Icone: IconeCheck, acento: "border-l-calm-line", corIcone: "bg-calm-bg text-calm" },
    { rotulo: "Em risco", valor: emRisco, Icone: IconeAlerta, acento: "border-l-ink-faint", corIcone: "bg-paper text-ink-soft" },
  ];

  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {cartoes.map((cartao) => (
        <div key={cartao.rotulo} className={`paper-card flex items-start justify-between rounded-sm border-l-4 p-4 ${cartao.acento}`}>
          <div className="min-w-0">
            <p className="eyebrow mb-1.5 truncate">{cartao.rotulo}</p>
            <p className="font-display text-2xl font-semibold text-ink">{cartao.valor}</p>
          </div>
