...
export default function LoginNutri() {
  ...
  const [estadoEntrar, acaoEntrar] = useFormState(entrarNutricionista, ESTADO_INICIAL);
  const [estadoCadastrar, acaoCadastrar] = useFormState(cadastrarNutricionista, ESTADO_INICIAL);
        ...
        <div className="mx-auto w-full max-w-sm">
          ...
          {modo === "entrar" ? (
            ...
            <form action={acaoCadastrar} className="mt-6 flex flex-col gap-4">
              ...
              <label className="text-sm">
                <span className="eyebrow mb-1.5 block">CRN (opcional)</span>
                <input
                  type="text"
                  name="crn"
                  className="w-full rounded-sm border border-rule bg-paper-raised px-3 py-2.5 text-sm outline-none transition-colors focus:border-brass"
